...
UCHAT.listen = function (mocket, config) {
    ...
    callbacks.after = config.callbacks.after || {};
    callbacks.before = config.callbacks.before || {};

    function tryBeforeCallback(name, client) {
        if (typeof callbacks.before[name] === 'function') {
            var args = [client],
                i;
            for (i = 1; i < arguments.length; i += 1) {
                args.push(arguments[i]);
            }
            return callbacks.before[name].apply(callbacks.before[name], args);
        }
        return true;
    }

    function afterCallback(name, client) {
        if (typeof callbacks.after[name] === 'function') {
            var args = [client],
                i;
            for (i = 1; i < arguments.length; i += 1) {
                args.push(arguments[i]);
            }
            return callbacks.after[name].apply(callbacks.after[name], args);
        }
    }
    ...
    mocket.on('$connection', function (client) {
        if (!tryBeforeCallback('$connection', client)) {
            return;
        }
        ...
        client.on('uchat-enter', function (roomName) {
            ...
            if (!tryBeforeCallback('uchat-enter', client, roomName)) {
                return;
            }
            ...
            room = rooms[roomName];
            client.room = room;
            client.roomIndex = room.clients.push(client);
            ...
            client.on('uchat-join', function (nameIn) {
                ...
                if (!tryBeforeCallback('uchat-join', client, nameIn)) {
                    return;
                }
            ...
            client.on('uchat-msg', function (msg) {
                if (typeof room.joined[name] !== 'undefined') {
                    ...
                    if (!tryBeforeCallback('uchat-msg', client, msg)) {
                        return;
                    }
                ...
                afterCallback('uchat-msg', client, msg);
            });
            ...
            client.on('$disconnect', function () {
                tryBeforeCallback('$disconnect', client);
                ...
                afterCallback('$disconnect', client);
            });

            afterCallback('uchat-enter', client);
        });
    });
